...
import { createCmdKey, commandsCtx, rootCtx, createCmd } from "@milkdown/core";
import { TextSelection, setBlockType, undo, redo, wrapIn, liftListItem, sinkListItem, PluginKey, selectParentNode, Plugin } from "@milkdown/prose";
  ...
  [
    {
      ...
      disabled: (view) => {
        ...
        return !(view.state.selection instanceof TextSelection) || !(setToHeading(1) || setToHeading(2) || setToHeading(3));
      },
...
const select = (utils, config, ctx, view) => {
  ...
  selector.addEventListener("mousedown", (e) => {
    ...
    selectorList.style.left = `${selectorWrapper.getBoundingClientRect().left - view.dom.getBoundingClientRect().left}px`;
  });
...
class Manager {
  constructor(originalConfig, utils, ctx, menu2, view) {
    this.utils = utils;
    this.ctx = ctx;
    this.config = originalConfig.map((xs) => xs.map((x) => __spreadProps(__spreadValues({}, x), {
      $: this.$create(x, view)
    }))).map((xs, i) => {
      ...
      return [...xs, __spreadProps(__spreadValues({}, dividerConfig), { $: this.$create(dividerConfig, view) })];
    }).flat();
  ...
  update(view) {
    this.config.forEach((config) => {
      ...
      if (config.type === "select") {
        if (config.disabled) {
          const disabled = config.disabled(view);
          const button2 = config.$.children[0];
          if (button2) {
            if (disabled) {
...
const defaultDOMHandler = ({ menu: menu2, menuWrapper, editorDOM, editorRoot }) => {
...
const menubar = (utils, view, ctx, domHandler = defaultDOMHandler) => {
...
const menuKey = new PluginKey("MILKDOWN_PLUGIN_MENU");
const menu = createPlugin((utils, options) => {
  var _a;
  const config = (_a = options == null ? void 0 : options.config) != null ? _a : defaultConfig;
  const domHandler = options == null ? void 0 : options.domHandler;
  return {
    ...
    prosePlugins: (_, ctx) => {
      const plugin = new Plugin({
        key: menuKey,
        view: (editorView) => {
          const menu2 = menubar(utils, editorView, ctx, domHandler);
